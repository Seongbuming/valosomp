#!/usr/bin/env python3
"""
Crisis Type Analysis (Full Dataset): Human-induced vs Natural Disasters
- Full 3,072 tweets with crisis categorization
...
print("=" * 80)
print("Crisis Type Analysis (Full Dataset): Human-induced vs Natural")
print("=" * 80)
...
# Load data with crisis categorization
df = pd.read_csv('emotional_and_intent/combined_analysis_data_with_crisis.csv')
...
print(f"  Mixed: {(df['crisis_category']=='Mixed').sum()} ({(df['crisis_category']=='Mixed').sum()/len(df)*100:.1f}%)")

# For primary analysis, exclude "Mixed" category
df_binary = df[df['crisis_category'].isin(['Human-induced', 'Natural'])].copy()
print(f"\nBinary analysis (excluding Mixed): {len(df_binary)} tweets")
...
for emotion in emotion_cols:
    human_mean = df_binary[df_binary['crisis_category']=='Human-induced'][emotion].mean()
    natural_mean = df_binary[df_binary['crisis_category']=='Natural'][emotion].mean()

    # Mann-Whitney U test
    human_data = df_binary[df_binary['crisis_category']=='Human-induced'][emotion]
    natural_data = df_binary[df_binary['crisis_category']=='Natural'][emotion]
...
for intent in intent_cols:
    human_mean = df_binary[df_binary['crisis_category']=='Human-induced'][intent].mean()
    natural_mean = df_binary[df_binary['crisis_category']=='Natural'][intent].mean()

    human_data = df_binary[df_binary['crisis_category']=='Human-induced'][intent]
    natural_data = df_binary[df_binary['crisis_category']=='Natural'][intent]
    stat, p_value = mannwhitneyu(human_data, natural_data, alternative='two-sided')
...
for dimension in ['I', 'You', 'We']:
    ...
    # Human-induced
    human_low = df_binary[(df_binary['crisis_category']=='Human-induced') & (df_binary[f'{dimension}_level']=='Low')]
    human_high = df_binary[(df_binary['crisis_category']=='Human-induced') & (df_binary[f'{dimension}_level']=='High')]

    # Natural
    natural_low = df_binary[(df_binary['crisis_category']=='Natural') & (df_binary[f'{dimension}_level']=='Low')]
    natural_high = df_binary[(df_binary['crisis_category']=='Natural') & (df_binary[f'{dimension}_level']=='High')]
...
for dimension in ['I', 'You', 'We']:
    ...
    crosstab = pd.crosstab(
        df_binary[f'{dimension}_level'],
        df_binary['crisis_category'],
        normalize='index'
    ) * 100
    ...
    # Chi-square test
    contingency = pd.crosstab(df_binary[f'{dimension}_level'], df_binary['crisis_category'])
...
output_dir = 'emotional_and_intent/crisis_type_results_full'
...
human_means = [df_binary[df_binary['crisis_category']=='Human-induced'][e].mean() for e in emotion_cols]
natural_means = [df_binary[df_binary['crisis_category']=='Natural'][e].mean() for e in emotion_cols]
...
ax.set_title('Emotion Patterns by Crisis Category (N=2,855)', fontsize=14, fontweight='bold')
ax.set_xticks(x)
...
human_means = [df_binary[df_binary['crisis_category']=='Human-induced'][i].mean() for i in intent_cols]
natural_means = [df_binary[df_binary['crisis_category']=='Natural'][i].mean() for i in intent_cols]
...
ax.set_ylabel('Mean Score', fontsize=12)
ax.set_title('Intent Patterns by Crisis Category (N=2,855)', fontsize=14, fontweight='bold')
ax.set_xticks(x)
...
for row_idx, crisis in enumerate(['Human-induced', 'Natural']):
    for col_idx, dimension in enumerate(['I', 'You', 'We']):
        ...
        crisis_df_subset = df_binary[df_binary['crisis_category'] == crisis]
...
for idx, dimension in enumerate(['I', 'You', 'We']):
    ...
    # Prepare data
    human_low = len(df_binary[(df_binary['crisis_category']=='Human-induced') & (df_binary[f'{dimension}_level']=='Low')])
    human_high = len(df_binary[(df_binary['crisis_category']=='Human-induced') & (df_binary[f'{dimension}_level']=='High')])
    natural_low = len(df_binary[(df_binary['crisis_category']=='Natural') & (df_binary[f'{dimension}_level']=='Low')])
    natural_high = len(df_binary[(df_binary['crisis_category']=='Natural') & (df_binary[f'{dimension}_level']=='High')])
...
print("\n" + "=" * 80)
print("Crisis Type Analysis (Full Dataset) Complete!")
print("=" * 80)
print(f"\n결과 저장 위치: {output_dir}/")
print("  - CSV: emotion_by_crisis_category.csv, intent_by_crisis_category.csv")
print("  - Figures: 5 PNG files")
print(f"\n분석 데이터: {len(df_binary)} tweets (Human-induced + Natural only)")
